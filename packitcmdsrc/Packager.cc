	...
	_error_count(0), _listener(0)
{
    ...
    licence("");
    copyright("");
	set_error(PackageItem::ITEM_TO_PACKAGE, "Item to package must be entered");
	set_error(PackageItem::INSTALL_TO, "Install location must be entered");
...
/// @brief return list of errors
/// @return list of error messages
std::vector<std::string> Packager::errors() const
{
	std::vector<std::string> errs;
	if (_error_count > 0)
	{
		int start = first_error();
		int next = start;
		do
		{
			errs.push_back("Control field '" + item_name(next) + "' " + _errors[next]);
			next = next_error(next);
		} while (next != start);
	}
	return errs;
}
...
void Packager::check_standards_version()
{
	...
	{
		bool comps_flags_set = false;
		for (ItemToPackage &itp : _items_to_package)
		{
			if (itp.component_flags != "None" )
			{
				comps_flags_set = true;
			}
		}
		if (comps_flags_set)
		{
...
{
	const std::set<std::string> c_valid_environments(
		{"any", "arm", "vfp", "vfpv3", "arm, swp", "arm26", "arm32"}
		);
   _environment = value;
   check_standards_version();
   if (value.empty())
   ...
   } else if (c_valid_environments.count(value) == -0)
   {
	  std::ostringstream os;
	  os << "Invalid value, must be one of";
	  for(const std::string &env : c_valid_environments)
	  {
		os << " \"" << env << "\"";
	  }
	  set_error(ENVIRONMENT, os.str());
   } else
   {
...
bool Packager::check_component_flags(const std::string &value)
{
	...
	if (c_valid_comp_flags.count(value) == 0)
	{
 	    std::ostringstream os;
	    os << "Invalid value, must be one of";
	    for(const std::string &flag : c_valid_comp_flags)
	    {
	 	  os << " \"" << flag << "\"";
	    }
...
/// @param item file/folder name of item to package
/// @param where location to install to
/// @param flags component flags "" or "None" for none.
...
//		Hourglass hg();
		...
		_items_to_package.clear(); // Clear new item default
		...
		for (int item = 0; item < num_items; item++)
		{
			...
			case SD_SYSVARS:
			case SD_SPRITES:
			    //TODO: report SYSVARS and sprites ignored
				break;
			}
		}

		set_payload(install_item);
		item_to_package("extract " + filename + " " + _items_to_package.back().item);

		// Install to must always be given a value otherwise a save
		// will not work.
		if (_items_to_package.back().install_to.empty()) install_to("Apps.Misc");
...
/// @brief Load a given control file
/// @param filename 
/// @return 
bool Packager::load_control(std::string filename)
{
	std::ifstream ctrl(filename.c_str());
	...
	catch(const PackageFormatException& e)
	{
		std::cerr << "Failed to read control file " << e.message() << std::endl;
		return false;
	}	
}

/// @brief Load copyright from a file
/// @param filename file name to load from
/// @return true if successfule
bool Packager::load_copyright(std::string filename)
{
	std::ifstream cpy_stream(filename.c_str());
	if (!cpy_stream)
	{
		std::cerr << "Unable to open copyright file" << std::endl;
		return false;
	}
	std::stringstream str_stream;
    str_stream << cpy_stream.rdbuf();//read the file
    copyright(str_stream.str());
	return true;
}
...
/**
 * Compare the passed tbx::Path with the item name and update it to be the
...
//		hg.percentage(1);
		...
		{			
			typedef std::pair<std::string, size_t> InstallLocation;
			typedef std::vector<std::pair<tbx::Path, tbx::PathInfo>> InstallFiles;
			std::map<InstallLocation, InstallFiles> install_list;

			for (const auto &item_to_package : _items_to_package)
			{
				tbx::Path files(item_to_package.item);
				size_t base_dir_size = 0;
				if (item_to_package.item.find('.') != std::string::npos)
				{
					base_dir_size = files.parent().name().length() + 1;
				}
				auto &file_list = install_list[std::make_pair(item_to_package.install_to, base_dir_size)];
			...
			for (const auto &install_files : install_list)
			{
				total_files += install_files.second.size();
			}
			for (const auto &install_files : install_list)
			{
				const std::string &install_to = install_files.first.first;
				size_t base_dir_size = install_files.first.second;
				const auto &files_to_copy = install_files.second;
				for (const auto &file_info : files_to_copy)
				{
					copy_file(zip, file_info.first, file_info.second, install_to, base_dir_size);
					unsigned int pc = ++files_done * 100 / total_files;
					if (pc > last_percent)
					{
						last_percent = pc;
					//	hg.percentage(pc);
	...
	{
		std::cerr << "Unable to allocate enough memory to create package" << std::endl;
	} catch(...)
	{
		std::cerr << "Unexpected exception thrown during package creation" << std::endl;
	}
...
void Packager::write_control(CZipArchive &zip) const
{
    ...
    for (const auto &item_to_package : _items_to_package)
	{
		if (!item_to_package.component_flags.empty() && item_to_package.component_flags != "None")
		{
			std::string::size_type leaf_pos = item_to_package.item.rfind('.');
			std::string leaf_name = (leaf_pos == std::string::npos) ? item_to_package.item : item_to_package.item.substr(leaf_pos+1);
			if (!components.empty()) components += ", ";
			components += item_to_package.install_to + "." + leaf_name 
				+ " (" + item_to_package.component_flags + ")";
		}
	}
	if (!components.empty())
	{
		os << "Components: " << components << std::endl;
	}
 ...
 */
void Packager::copy_file(CZipArchive &zip, const tbx::Path &filename, const tbx::PathInfo &entry, const std::string &install_to, size_t base_dir_size) const
{
	// swap "." and slashes in name to put in zip file
	std::string nameinzip = install_to + "." + filename.name().substr(base_dir_size);
...
bool Packager::files_in_zip(std::string &zipfile, std::string &zipitem) const
{
	for (const auto &item_to_package : _items_to_package)
	{
		if (item_to_package.item.substr(0, 8) != "extract ") return false;

		zipfile = item_to_package.item.substr(8);
		...
		if (zipitem.empty() || zipfile.empty())
		{
			return false;
		}
	}
...
bool Packager::copying_from_zip(const std::string &filename, std::string &oldzipname, std::string &oldzipitem, bool &delete_old_zip)
{
	...
	bool copying_from_zip = false;
	for (const auto &item_to_package : _items_to_package)
	{
        copying_from_zip |= (item_to_package.item.substr(0, 8) == "extract ");
	}
	if (!copying_from_zip) return false;
	if (_items_to_package.size() != 1)
	{
		throw PackageCreateException("Can only have one item to package when copy from previous zip file");
	}
...
//   hg.percentage(2);
   ...
   {
  	  oldzip.Close();
	  throw PackageCreateException("Only able to copy from one location from the old zip file");
   }

   std::string newloc = riscos_to_zip_name(_items_to_package.back().install_to);
